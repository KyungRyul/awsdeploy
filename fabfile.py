...
def setup():
    # 리눅스 업데이트 및 업그레이드
    _get_latest_apt()
    # 서비스에 구동에 필요한 os레벨상의 패키지를 설치해라
    _install_apt_requirements(apt_requirements)
    # 가상환경을 사용할 수 있는 패키지 설치
    _make_virtualenv()
# 최초 배포, 업그레이드 배포
# 업그레이드 배포시 명령
# zhem tnwjd -> 로컬테스트 완료 -> git커밋 , push -> fab deploy
# $> fab deploy
def deploy():
...
def _update_virtualenv():
    ...
    if not exists(virtualenv_folder + '/bin/pip'):
        ...
        # $> cd virtualenv awsdeploy : 가상환경을 만들어라
        run('cd /home/%s/.virtualenvs && virtualenv %s' % (env.user, PROJECT_NAME))
    # 해당 가상환경 안에 flask, scikit-learn 2개가 설치 될것이다.
